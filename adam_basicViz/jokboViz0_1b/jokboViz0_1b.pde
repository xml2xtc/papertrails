...
PShape mapBG;                // map backgrtound image
...
void setup() {
  ...
  datafile = loadStrings("jokboadam.csv"); // load the jokbo csv
...
class Adventure {
  PVector start;                      // vector for holding the start position ---how do I get the start position
  ...
  float distanceCol;                      // property for holdin the dstance color
  float distanceCol2;                      // property for holdin the dstance color
 
   
   Adventure(int year, float xO, float yO, float xD, float yD) {
     start = new PVector(xO,yO);           
   ...
   //the distance between the start vector(point) and the destination vector
   void adventureMove () {
     distanceCol = current.dist(destination) * 5; //starting distance colors
     distanceCol2 = current.dist(destination) * 2;
     if (current.dist(destination) > .1 ) {          // if the distance between the current position and the destination is more than .1, keep moving it
       ...
       noStroke();
       fill(distanceCol,distanceCol2,255); 
       ellipse(current.x,current.y,10,10);            // draw an ellipse where we are
     } else { 
       fill(distanceCol,distanceCol,255);
