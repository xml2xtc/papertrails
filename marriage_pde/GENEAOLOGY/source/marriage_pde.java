...
float overlayAlpha = 0, agentsAlpha = 100, strokeWidth = .5f, agentWidthMin = 1.5f, agentWidthMax = 15;
int drawMode = 1;
PShape img;
PShape img1;
int currentYear;
...
public void setup() {
  ...
  csvJokbo = new Jokbo("jokbo1cleaned.txt");
  clan0Y = map(clan0Y, 38.72f, 32.8f, 0, height);
  clan0X = map(clan0X, 125.77f, 129.73f, 0, width);
...
//      rect(clan0X - 50, clan0Y +400, 100, 40);
      ...
      while (currentYear == csvJokbo.getCurrentYear ()) {
        ...
        lat = map(lat, 38.72f, 32.8f, 0, height);        
        lo = map(lo, 125.77f, 129.73f, 0, width);
...
public void setNoiseZRange(float theNoiseZRange) {
  ...
  private void drawBride(float brideLocX, float brideLocY, float shapeSize) {
    ...
    vertex(-shapeSize/.732f, -0.5f*shapeSize);
...
class Jokbo {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--full-screen", "--bgcolor=#666666", "--hide-stop", "marriage_pde" };
    if (passedArgs != null) {
